...
class ClipExtractor:
    ...
    def _add_datetime_overlay(self, video_file: str, alert_time: datetime.datetime) -> str:
        """
        Add datetime label overlay to video at top right corner
        
        Args:
            video_file: Path to input video file
            alert_time: Datetime object to display on video
            
        Returns:
            Path to output video file with overlay (same path as input, overwrites)
        """
        # Format datetime as "DD-MM-YYYY HH:MM"
        datetime_str = alert_time.strftime('%d-%m-%Y %H:%M')
        
        logging.info(f"Adding datetime overlay '{datetime_str}' to video...")
        
        # Create temporary output file
        temp_output = video_file.replace('.mp4', '_with_overlay.mp4')
        # Create temporary text file for drawtext (more reliable with special characters)
        text_file = video_file.replace('.mp4', '_datetime_text.txt')
        
        try:
            # Write datetime string to text file
            with open(text_file, 'w', encoding='utf-8') as f:
                f.write(datetime_str)
            
            # Convert path to use forward slashes for ffmpeg (works on both Windows and Unix)
            text_file_path = text_file.replace('\\', '/')
            
            # Use ffmpeg drawtext filter to add datetime at top right
            # x = w - text_w - 10: positions text 10 pixels from right edge
            # y = 10: positions text 10 pixels from top edge
            # fontsize: 24, fontcolor: white, box: black background with padding
            # Using textfile option for reliable handling of special characters
            drawtext_filter = f"drawtext=textfile='{text_file_path}':fontsize=24:fontcolor=white:x=w-text_w-10:y=10:box=1:boxcolor=black@0.5:boxborderw=5"
            ...
            # Replace original file with overlay version
            os.replace(temp_output, video_file)
            logging.info(f"Successfully added datetime overlay to video")
            ...
            logging.error(f"Failed to add datetime overlay: {e.stderr}")
            ...
            logging.warning("Continuing with original video file (overlay failed)")
            return video_file
        except subprocess.TimeoutExpired:
            logging.error("FFmpeg timeout while adding datetime overlay")
            if os.path.exists(temp_output):
                try:
                    os.remove(temp_output)
                except Exception:
            ...
            logging.error(f"Unexpected error adding datetime overlay: {e}")
            if os.path.exists(temp_output):
                try:
                    os.remove(temp_output)
                except Exception:
